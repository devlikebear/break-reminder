...
func TestMenuBarCmdReturnsHelpfulErrorWhenHelperMissing(t *testing.T) {
	...
	if err.Error() != "break-menubar helper not found. If you installed via Homebrew, run 'brew reinstall break-reminder'. Otherwise run 'make build' or 'make install' so helpers are placed next to the break-reminder binary" {
