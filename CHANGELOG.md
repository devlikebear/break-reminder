...
### Added
- Homebrew installations can register a dedicated daily LaunchAgent that refreshes the tap, detects outdated releases, upgrades `devlikebear/tap/break-reminder`, and reloads the timer and menu bar agents.
- Added `break-reminder update` for an immediate manual Homebrew update check.

### Changed
- Homebrew service installation now writes stable `bin` symlink paths into LaunchAgent plists so jobs continue working when versioned Cellar directories are replaced.
- `service start`, `stop`, `status`, and `uninstall` now include the automatic updater lifecycle.
